...
import { plateBends, plateOutline } from "../../engine/api/project/plate-sketch-relations-and-bends.mjs?v=plate-outline-relation-safety-1";
...
function renderVisibilityEnabled(key) {
  return settings?.render?.visibility?.[key] !== false;
}

function shouldRenderCuttingObjects() {
  return renderVisibilityEnabled("cuttingObjects");
}

function shouldRenderReferencePlanes() {
  return renderVisibilityEnabled("referencePlanes");
}

function shouldRenderFasteners() {
  return renderVisibilityEnabled("fasteners");
}
...
function addPlateLikeCsgSolid(scene, polygons, color, edgeColor, meta = {}) {
...
function cutBodyPlateLikePrism(body, color) {
  if (body.type === "box") {
  ...
  return cutBodyPolygons(body, { color });
}
...
function addCutBody(scene, project, profiles, feature) {
  ...
  const polygons = bodies.flatMap((body) => cutBodyPlateLikePrism(body, display.color));
...
function addTrimJoint(scene, project, profiles, trimJoint) {
  ...
  const renderCuttingObjects = shouldRenderCuttingObjects();
  const renderReferencePlanes = shouldRenderReferencePlanes();
  const display = {
    color: "#ff3366",
    edgeColor: "#be123c",
    transparent: true,
...
function addFastenerGroups(scene, project, fastenerGroups = collectionObjects(project, "fastenerGroups")) {
  if (!shouldRenderFasteners()) return;
