...
function requiredVec3(value, label) {
  if (!Array.isArray(value) || value.length !== 3 || value.some((item) => !finiteNumber(item))) {
    throw new Error(`dimension context: ${label} must be a finite [x, y, z] vector`);
  }
  return value;
}

function requiredAxis(value, label) {
  const axis = v.norm(requiredVec3(value, label));
  if (v.len(axis) <= EPSILON) throw new Error(`dimension context: ${label} must have non-zero length`);
  return axis;
}
...
export function patternLayoutBasis(pattern, fallbackBasis) {
  ...
  if (!ref) return fallbackBasis;
  return {
    origin: requiredVec3(ref.origin, "layoutReference.origin"),
    normal: fallbackBasis.normal,
    localAxisY: requiredAxis(ref.localAxisY, "layoutReference.localAxisY"),
    localAxisZ: requiredAxis(ref.localAxisZ, "layoutReference.localAxisZ")
  };
}
