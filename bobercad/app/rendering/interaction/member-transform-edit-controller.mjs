...
export function createMemberEditController({ viewer, api, selection, snapManager, settings = {}, onLocalProjectChange, onMemberSelected, onCleared, onMessage, onTransformChange, autoRelationsEnabled = () => false, perfMark = () => {}, now = () => Date.now() }) {
  ...
  let lastDragSnapTime = 0;
  let lastDragSnapDurationMs = 0;
  let forceNextDragSnap = false;
  ...
  const smartRebuildDelayMs = finiteNumber(authoringSettings.smartRebuildDelayMs)
    ...
    : 500;
  const dragSnapRadius = finiteNumber(authoringSettings.memberEditDragSnapRadius)
    ? Math.max(0, authoringSettings.memberEditDragSnapRadius)
    : 6000;
  const maxDragSnapIntervalMs = finiteNumber(authoringSettings.memberEditDragSnapMaxIntervalMs)
    ? Math.max(0, authoringSettings.memberEditDragSnapMaxIntervalMs)
    : 80;
  ...
  function dragSnapContext(context = {}, rawPoint = null) {
    if (!v.isVec3(rawPoint)) return context;
    return {
      ...context,
      center: rawPoint,
      radius: finiteNumber(context.radius) ? context.radius : dragSnapRadius,
      maxMemberCandidates: finiteNumber(context.maxMemberCandidates) ? context.maxMemberCandidates : 80,
      maxPlateCandidates: finiteNumber(context.maxPlateCandidates) ? context.maxPlateCandidates : 80,
      maxIntersectionSources: finiteNumber(context.maxIntersectionSources) ? context.maxIntersectionSources : 16,
      maxVisibleCandidates: finiteNumber(context.maxVisibleCandidates) ? context.maxVisibleCandidates : 48,
      maxDiagnostics: finiteNumber(context.maxDiagnostics) ? context.maxDiagnostics : 40
    };
  }

  function snapAtCursor(screen, context = {}, event = null, rawPoint = null) {
  ...
  function dragSnapIntervalMs() {
    if (lastDragSnapDurationMs <= 12) return 0;
    return clamp(lastDragSnapDurationMs * 2, 24, maxDragSnapIntervalMs);
  }
  ...
  function dragSnapAtCursor(screen, context = {}, event = null, rawPoint = null) {
    ...
    if (lastDragSnapDurationMs > 12) {
      perfMark("member-drag-snap-slow", {
        durationMs: lastDragSnapDurationMs,
        intervalMs: dragSnapIntervalMs()
      });
    }
    return snap;
  }
  ...
  function cursorDepthPoint(basePoint, input, context) {
    ...
    const snap = dragSnapAtCursor(input.screen, dragSnapContext(context, point), input.event, point);
  ...
  function axisSnap(basePoint, axis, rawPoint, context, screen, event = null) {
    const snap = dragSnapAtCursor(screen, dragSnapContext(context, rawPoint), event, rawPoint);
  ...
  function draftFromDrag(input) {
    ...
    if (kind === "move-member") {
      ...
      const snap = dragSnapAtCursor(input.screen, dragSnapContext(drag.snapContext, rawCenter), input.event, rawCenter);
