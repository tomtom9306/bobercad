...
function planeHitPoint(viewer, candidate, screen, rawPoint) {
  ...
  if (ray) {
    const rayObject = requiredObject(ray, "viewer screen ray");
    point = linePlaneIntersection(
      requiredVec3(rayObject.origin, "viewer screen ray origin"),
      requiredVec3(rayObject.direction, "viewer screen ray direction"),
      origin,
      normal
    );
  }
...
function linePairIntersection(viewer, left, right, screen, tolerance) {
  ...
  if (closest.distance <= worldTolerance) {
    ...
    return {
      ...
      priority: Math.max(optionalNumber(left.priority, 0, `${left.candidateId}.priority`), optionalNumber(right.priority, 0, `${right.candidateId}.priority`)) + 80,
...
function preferredInteriorPlane(hits) {
  return hits.find((hit) => {
    ...
    return !hits.some((other) => (
      other !== hit
      && other?.kind !== "plane"
      && hitReferencesObject(other, hit.objectId)
...
function applyVisibilityFilter(hits, visibilityFilter, viewer, screen, diagnostics, diagnosticLimit, maxAccepted = Infinity) {
...
export function solveSnap({
  ...
  if (typeof viewer.projectPoint !== "function") fail("viewer.projectPoint is required");
  ...
  for (const candidate of usable) {
    ...
    const hit = candidate.kind === "line"
      ? lineHit(viewer, candidate, screen, snapScreenTolerance)
      : candidate.kind === "plane"
        ? planeHit(viewer, candidate, screen, rawPoint, snapScreenTolerance)
        : pointHit(viewer, candidate, screen, snapScreenTolerance);
    if (hit) {
      hits.push(hit);
    } else {
  ...
  const visibleHits = promotePreferredInteriorPlane(
    applyVisibilityFilter(hits, visibilityFilter, viewer, screen, rejectedDiagnostics, rejectedDiagnosticLimit, visibleCandidateLimit)
  );
