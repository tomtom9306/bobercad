...
function requiredArrayInput(ctx, path, label) {
  ...
  if (!Array.isArray(value)) {
    ctx.error("stair-railing-input-invalid", `${label} must be an array.`, { parameterPaths: [path] });
    return undefined;
  }
  return value;
}
...
function createWallHandrailBracket(ctx, side, index, station, railPoint, profile) {
  ...
  const plate = ctx.part.plate(plateRole, {
...
function createWallMountingSurfaces(ctx, side, stations, basePoints, topPoints) {
  ...
  for (let index = 0; index < basePoints.length - 1; index += 1) {
    ...
    const surface = ctx.part.plate(role, {
...
export function buildRailingSystem(ctx, options = {}) {
  const stations = requiredArrayInput(ctx, "layout.railStations", "Solved railing stations");
  ...
  if (!stations || !width || !height || !sideSetting || sideInset === undefined || !postProfile || !railProfile || !requestedInfill) return;
  ...
  for (const side of sideList) {
    ...
    if (infill === "mid-rails" && options.family !== "wall-handrail") {
      ...
      for (let index = 0; index < basePoints.length - 1; index += 1) {
        ...
        const panel = ctx.part.plate(panelRole, {
        ...
        const fixingId = createPanelFixings(ctx, side, index, panel, [postIds[index], postIds[index + 1]], span, panelHeight);
