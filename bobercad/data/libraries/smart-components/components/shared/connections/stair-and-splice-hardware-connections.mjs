...
function plateSketchBounds(plate) {
  if (!plate.sketch || typeof plate.sketch !== "object" || Array.isArray(plate.sketch)) throw new Error(`${plate.id}: sketch is required`);
  if (!Array.isArray(plate.sketch.vertices)) throw new Error(`${plate.id}: sketch.vertices must be an array`);
  const points = plate.sketch.vertices
    .map((vertex) => vertex.point)
    .filter((point) => Array.isArray(point) && point.length >= 2);
  if (!points.length) throw new Error(`${plate.id}: sketch.vertices must include points`);
  return {
...
function createMemberSplitTrim(ctx, baseRole, member, center, frame, splitFrame, cutGap, size) {
  ...
  const span = Math.max(size.width, size.height, cutGap) * 2;
...
function createHoleFeatureAndFasteners(ctx, baseRole, plate, positions, options = {}) {
  ...
  const patternRole = registerRole(ctx, `${baseRole}Pattern`, `_${baseRole}_pattern`);
  const featureRole = registerRole(ctx, `${baseRole}Holes`, `_${baseRole}_holes`);
...
function createTreadCleats(ctx, treads, supports, generatedIds) {
  ...
  for (const [index, tread] of treads.entries()) {
    ...
    for (const [sideIndex, support] of sideSupports.entries()) {
      ...
      const plate = ctx.part.plate(registerRole(ctx, roleBase, `_${roleBase}`), {
...
function createMountingSurfaces(ctx, floorMembers, topMembers, frame, generatedIds) {
  ...
  if (floorPoints.length) {
    ...
    const floor = ctx.part.plate(registerRole(ctx, "floorMountingSurface", "_floor_mounting_surface"), {
  ...
  if (topPoints.length) {
    ...
    const topSlab = ctx.part.plate(registerRole(ctx, "topSlabMountingSurface", "_top_slab_mounting_surface"), {
...
function createFloorAndSlabFixings(ctx, supports, generatedIds) {
  ...
  for (const [index, member] of supports.entries()) {
    ...
    if (isFloorStart) {
      ...
      const startPlate = ctx.part.plate(registerRole(ctx, startRole, `_${startRole}`), {
    ...
    if (isTopEnd) {
      ...
      const topPlate = ctx.part.plate(registerRole(ctx, endRole, `_${endRole}`), {
...
function createSpiralColumnFixings(ctx, column, generatedIds) {
  ...
  if (ctx.input("connections.showMountingSurfaces") !== false) {
    ...
    const floorSurface = ctx.part.plate(registerRole(ctx, "spiralFloorMountingSurface", "_spiral_floor_mounting_surface"), {
    ...
    const topSurface = ctx.part.plate(registerRole(ctx, "spiralTopMountingSurface", "_spiral_top_mounting_surface"), {
  ...
  const basePlate = ctx.part.plate(registerRole(ctx, baseRole, "_spiral_column_base_plate"), {
  ...
  const topPlate = ctx.part.plate(registerRole(ctx, topRole, "_spiral_column_top_plate"), {
...
function createSpiralTreadColumnBrackets(ctx, treads, column, generatedIds) {
  ...
  for (const [index, tread] of treads.entries()) {
    ...
    const bracket = ctx.part.plate(registerRole(ctx, roleBase, `_spiral_tread_column_bracket_${index + 1}`), {
...
function createPostBaseFixings(ctx, posts, generatedIds) {
  ...
  for (const [index, post] of posts.entries()) {
    ...
    const plate = ctx.part.plate(registerRole(ctx, role, `_${role}`), {
...
export function buildStairHardwareConnections(ctx, options = {}) {
  ...
  ctx.objectPattern.create("connectionPattern", {
    ...
    transform: {
      kind: "stair-hardware",
      family: options.family || "stair-hardware",
