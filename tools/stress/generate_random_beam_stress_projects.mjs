...
function makeMember(index, count, rng) {
  ...
  return {
    ...
    authoring: {
      source: "tools/stress/generate_random_beam_stress_projects.mjs",
...
function projectPrefix(outputDir, label, count) {
  ...
  const framesPath = rel(outputDir, path.join(ROOT, "bobercad", "data", "libraries", "frames", "frame-register.json"));
  ...
  return {
    ...
    project: {
      ...
      createdWith: "tools/stress/generate_random_beam_stress_projects.mjs",
      bim: {
        name: `Stress Test ${label.toUpperCase()} Beams`,
        propertySets: {
          StressTest: {
