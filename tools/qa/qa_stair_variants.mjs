...
import { plateOutline } from "../../bobercad/app/engine/api/project/plate-sketch-relations-and-bends.mjs";
...
function validateReusableConnectionKinds(project, issues, demoId) {
  ...
  if (top && stairIntent(project).connections !== "none" && !connectionChildren.some((instance) => instance.type === "stair-hardware")) {
    addIssue(issues, "error", "stair-hardware-connection-missing", `${demoId}: stair system must select stair-hardware instead of stair-only connection families`);
  }
}
