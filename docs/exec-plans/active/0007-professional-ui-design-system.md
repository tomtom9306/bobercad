...
- Datum authoring now treats Grid Systems and Levels as normal indexed model collections. `model.grid.create` and `model.level.create` create store-backed datum objects, the Model Browser exposes `gridSystems` and `levels`, generated Properties edits grid name/origin/basis/axis labels/positions and level name/elevation, and render visibility includes a dedicated Grids toggle.
