...
- Agent `USER_REVIEW` notes must include the dedicated preview URL from that agent worktree. The URL must use the agent port (`5181`-`5190`) and include the branch path prefix immediately after the host, for example `/codex/agentN/...`; otherwise the user may be reviewing `main` instead of the agent's work.
