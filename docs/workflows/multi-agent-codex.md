...
- The preview URL given to the user must include the branch path prefix printed by the server immediately after the host, for example `/codex/agent2/...`. A review URL on port `5173` or without the agent branch path prefix is invalid for agent work.
